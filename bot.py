...
class MultiPairScalpingTrader:
    def __init__(self):
        ...
        # UPDATED SCALPING parameters - $200 per trade, 5x leverage
        self.trade_size_usd = 200  # $200 per trade
    ...
    def get_quantity(self, pair, price):
        """SIMPLE quantity calculation - $200 fixed size"""
        try:
            # Always use $200 trade size
            ...
            if trade_value < 20:
                print(f"❌ Below Binance minimum: ${trade_value:.2f}")
                return None
    ...
    def execute_scalping_trade(self, decision):
        """UNIVERSAL TRADE EXECUTION FOR ALL PAIRS"""
        try:
            pair = decision["pair"]
