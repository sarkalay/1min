...
class AIPaperTradingBot:
    def __init__(self, real_bot):
        ...
        self.real_bot.print_color("🤖 AI PAPER TRADING BOT INITIALIZED!", Fore.GREEN + Style.BRIGHT)
        self.real_bot.print_color(f"💰 Starting Paper Balance: ${self.paper_balance}", Fore.CYAN)
    ...
    def paper_execute_trade(self, decision):
        ...
        try:
            ...
            ticker = self.real_bot.binance.futures_symbol_ticker(symbol=pair)
            current_price = float(ticker['price'])
            ...
            # TP/SL calculation (မင်း bot ထဲက logic အတိုင်း)
            if direction == "LONG":
                tp_raw = current_price * 1.008
                sl_raw = current_price * 0.995
            else:
    ...
    def get_paper_portfolio_status(self):
        ...
        self.real_bot.print_color(f"\n💼 PAPER TRADING PORTFOLIO", self.real_bot.Fore.CYAN + Style.BRIGHT)
    ...
    def show_paper_trade_history(self, limit=10):
        ...
        self.real_bot.print_color(f"\n📋 PAPER TRADE HISTORY (Last {min(limit, len(self.paper_history))} trades)", self.real_bot.Fore.CYAN)
        self.real_bot.print_color("=" * 80, self.real_bot.Fore.CYAN)
    ...
    def run_paper_trading_cycle(self):
        ...
        try:
            ...
            if market_data:
                self.real_bot.print_color(f"\n🤖 AI SCANNING FOR PAPER TRADES...", self.real_bot.Fore.BLUE + Style.BRIGHT)
                ...
                for pair in market_data.keys():
                    ...
                    if pair not in self.paper_positions and len(self.paper_positions) < self.real_bot.max_concurrent_trades:
                        ...
                        if decision["action"] == "TRADE":
                            self.real_bot.print_color(f"🎯 AI PAPER SIGNAL: {pair} {decision['direction']} ({decision['confidence']}%)", self.real_bot.Fore.GREEN + Style.BRIGHT)
            ...
            self.real_bot.print_color(f"Paper trading cycle error: {e}", self.real_bot.Fore.RED)

    def start_paper_trading(self):
        """Start AI paper trading"""
        self.real_bot.print_color("🚀 STARTING AI PAPER TRADING BOT!", self.real_bot.Fore.GREEN + Style.BRIGHT)
        self.real_bot.print_color("📝 REAL AI ANALYSIS + PAPER EXECUTION", self.real_bot.Fore.CYAN)
        ...
        while True:
            try:
                cycle_count += 1
                self.real_bot.print_color(f"\n🔄 PAPER TRADING CYCLE {cycle_count}", self.real_bot.Fore.CYAN)
                ...
                self.real_bot.print_color(f"\n🛑 PAPER TRADING STOPPED", self.real_bot.Fore.RED + Style.BRIGHT)
...
if __name__ == "__main__":
    try:
        ...
        real_bot = RealOrderPositionTracker()
        
        print("\n" + "="*60)
        print("🤖 FINAL AI TRADING BOT")
        print("="*60)
        print("SELECT TRADING MODE:")
        print("1. 🔴 Live Trading (Real Money - RISKY)")
        print("2. 🟢 AI Paper Trading (RECOMMENDED - NO RISK)")
        ...
        if choice == "1":
            ...
            if confirm.upper() == 'YES':
                ...
                paper_bot = AIPaperTradingBot(real_bot)
                paper_bot.start_paper_trading()
        else:
            # Default to AI Paper Trading
            paper_bot = AIPaperTradingBot(real_bot)
            paper_bot.start_paper_trading()
